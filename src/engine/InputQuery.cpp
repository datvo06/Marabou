...
InputQuery::InputQuery() : _networkLevelReasoner(NULL) {}
...
void InputQuery::addPiecewiseLinearConstraint(
...
const List<PiecewiseLinearConstraint *> &
InputQuery::getPiecewiseLinearConstraints() const {
  return _plConstraints;
}

void InputQuery::addTranscendentalConstraint(
...
const List<TranscendentalConstraint *> &
InputQuery::getTranscendentalConstraints() const {
  return _tsConstraints;
}
...
void InputQuery::removeEquation(Equation e) { _equations.erase(e); }
...
InputQuery &InputQuery::operator=(const InputQuery &other) {
  ...
  if (!other._networkLevelReasoner) {
    ...
    INPUT_QUERY_LOG(
        Stringf("Number of piecewise linear constraints in input query: %u",
                other._plConstraints.size())
            .ascii());
    INPUT_QUERY_LOG(
        Stringf("Number of transcendental constraints in input query: %u",
                other._tsConstraints.size())
            .ascii());
    INPUT_QUERY_LOG(
        Stringf(
            "Number of piecewise linear constraints in topological order %u",
    ...
    ASSERT(
        other._networkLevelReasoner->getConstraintsInTopologicalOrder().size() +
            numberOfDisjunctions + numberOfMaxs ==
        other._plConstraints.size());
...
InputQuery::InputQuery(const InputQuery &other) : _networkLevelReasoner(NULL) {
  *this = other;
}
...
void InputQuery::saveQuery(const String &fileName) {
  ...
  queryFile->write(
      Stringf("%u", _plConstraints.size() + _tsConstraints.size()));
...
void InputQuery::printInputOutputBounds() const {
  ...
  for (const auto &pair : _variableToInputIndex) {
    printf("\tInput %u (var %u): [%lf, %lf]\n", pair.second, pair.first,
           _lowerBounds[pair.first], _upperBounds[pair.first]);
  }
...
void InputQuery::dump() const {
  ...
  for (unsigned i = 0; i < _numberOfVariables; ++i) {
    printf("\t %u: [%s, %s]\n", i,
           _lowerBounds.exists(i) ? Stringf("%lf", _lowerBounds[i]).ascii()
                                  : "-inf",
           _upperBounds.exists(i) ? Stringf("%lf", _upperBounds[i]).ascii()
                                  : "inf");
  }
...
void InputQuery::adjustInputOutputMapping(
    const Map<unsigned, unsigned> &oldIndexToNewIndex,
...
bool InputQuery::constructNetworkLevelReasoner() {
  ...
  while (
      constructWeighedSumLayer(nlr, handledVariableToLayer, newLayerIndex) ||
  ...
  if (success) {
    ...
    INPUT_QUERY_LOG(
        Stringf(
            "successful. Constructed %u layers with %u neurons (out of %u)\n",
            newLayerIndex, count, getNumberOfVariables())
            .ascii());
...
bool InputQuery::constructWeighedSumLayer(
    NLR::NetworkLevelReasoner *nlr,
    Map<unsigned, unsigned> &handledVariableToLayer, unsigned newLayerIndex) {
  ...
  public:
    NeuronInformation(unsigned variable, unsigned neuron, const Equation *eq)
        : _variable(variable), _neuron(neuron), _eq(eq) {}

    NeuronInformation() : _eq(NULL) {}
  ...
  for (const auto &eq : equations) {
    ...
    if (eqVariables.size() == 1) {
      ...
      newNeurons.append(
          NeuronInformation(*eqVariables.begin(), newNeurons.size(), &eq));
    }
  }
  ...
  for (const auto &newNeuron : newNeurons) {
    ...
    */
    ...
    ASSERT(!FloatUtils::isZero(
        newNeuron._eq->getCoefficient(newNeuron._variable)));
    double factor = -1.0 / newNeuron._eq->getCoefficient(newNeuron._variable);

    // Bias
    nlr->setBias(newLayerIndex, newNeuron._neuron,
                 factor * -newNeuron._eq->_scalar);
    ...
    for (const auto &addend : newNeuron._eq->_addends) {
      ...
      nlr->setWeight(sourceLayer, sourceNeuron, newLayerIndex,
                     newNeuron._neuron, factor * addend._coefficient);
    }
  }
...
bool InputQuery::constructReluLayer(
    NLR::NetworkLevelReasoner *nlr,
    Map<unsigned, unsigned> &handledVariableToLayer, unsigned newLayerIndex) {
  INPUT_QUERY_LOG("Attempting to construct ReluLayer...");
  struct NeuronInformation {
  public:
    NeuronInformation(unsigned variable, unsigned neuron,
                      unsigned sourceVariable)
        : _variable(variable), _neuron(neuron),
  ...
  for (const auto &newNeuron : newNeurons) {
    ...
    nlr->addActivationSource(sourceLayer, sourceNeuron, newLayerIndex,
                             newNeuron._neuron);
  }
...
bool InputQuery::constructSigmoidLayer(
    NLR::NetworkLevelReasoner *nlr,
    Map<unsigned, unsigned> &handledVariableToLayer, unsigned newLayerIndex) {
...
bool InputQuery::constructSignLayer(
    NLR::NetworkLevelReasoner *nlr,
    Map<unsigned, unsigned> &handledVariableToLayer, unsigned newLayerIndex) {
...
bool InputQuery::constructMaxLayer(
    NLR::NetworkLevelReasoner *nlr,
    Map<unsigned, unsigned> &handledVariableToLayer, unsigned newLayerIndex) {
  ...
  for (const auto &plc : plConstraints) {
    ...
    newNeurons.append(
        NeuronInformation(f, newNeurons.size(), max->getElements()));
    nlr->addConstraintInTopologicalOrder(plc);
  }
